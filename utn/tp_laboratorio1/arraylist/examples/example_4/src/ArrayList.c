...
int al_clear(ArrayList* this)
{
    ...
    {
        returnAux=0;
        ArrayList *pAux=realloc(this->pElements,sizeof(void*)*AL_INITIAL_VALUE);
        if(pAux!=NULL)
        {
            this->size=0;
            free(this->pElements);
...
{
    int returnAux = -1;
    if(this!=NULL && pElement!=NULL && index>=0 && index<=this->size && !expand(this,index))
    {
        this->pElements[index]=pElement;
        returnAux=0;
        this->size++;
...
{
    int returnAux = -1;
    if(this!=NULL && index>=0 && index<=this->size && (!resizeUp(this)) )
    {
        int i;
